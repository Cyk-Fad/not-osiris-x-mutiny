...
void GUI::renderMenuBar() noexcept
{
	...
	ImGui::SetNextWindowPos({ 0.f, ((skreen_height / 2) - 154) });
	ImGui::Begin("Side tabb", NULL, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar);
	{
		...
		if (ImGui::Button("TRIGGERBOT", ImVec2(120, 45)))
		{
	...
	if (window.config) {
		...
		if (ImGui::ListBox("", &currentConfig, [](void* data, int idx, const char** out_text) {
			...
			*out_text = vector[idx].c_str();
			...
			strcpy_s(buffer, configItems[currentConfig].c_str());
